...
历史 trial
  ...
  → 写回历史，进入下一轮
  → 本次命令的 trial 批次全部结束后，Summary Agent 汇总已执行尝试并更新历史总结索引
```

第一轮不调用 Proposal/Feasibility，直接运行 `base_parameters.json` 建立真实基线。Summary Agent 要等本次命令请求的全部 trial 结束后才调用。
...
| Summary | 本次命令的真实 trial 批次结束后，根据该日期目录的真实 trial 总结问题、有效方向和无效方向 |

Summary Agent 不参与候选生成，也没有工具。它只消费脚本构造的去重事实：实际执行的参数修改、Proposal hypothesis、`reference_trial_id`、阶段指标、资源与终止事实。`run_context` 由确定性代码从基线参数提取并注入最终结果，包含算法、模型、训练/评测数据集、平台和关键 workload；Summary Agent 不自行推断这些字段。

Summary Agent 属于非阻塞的命令收尾步骤。一次 `run_circle.sh` 不论实际完成一个还是多个 trial，都只在本次 trial 批次结束后调用一次；如果一个 trial 都没有实际执行，则不调用。它没有启停开关，也不增加额外语义校验。`summary_max_output_tokens` 单独控制它较长上下文下的最大输出 token。成功时只发布 `summary/summary_result.json` 并更新全局索引，对话 trace 合并进本次命令最后完成 trial 的 `agent_trace.json`；调用失败只发送本次运行事件，不保存错误、状态或调试文件，也不会改写训练状态。
...
| `query_tuning_summaries` | Proposal 按阶段及关键词查询其他 run 的 Summary Agent 结果；返回正反经验及原始运行上下文 |
...
├── summary/
│   └── summary_result.json       # 本 run 截至最近一次命令结束的权威总结
└── trials/NNNN/
    ├── parameters.json
    ...
    ├── log_facts.json
    ├── decision.json
    ├── agent_trace.json          # 每批最后一个 trial 的 trace 含 Summary Agent
    ├── command.json
    ├── train.log
    ├── gpu_samples.csv
...
所有日期目录之外还有 `output/summary_index.json`。它不复制完整总结，只记录 `run_context`、可用阶段和权威结果相对路径；每次 trial 批次结束、当前 run 的 Summary Agent 成功后更新。系统不会自动总结或导入旧的 `summary_replays`；需要把已有 run 变成正式经验时，显式运行 `tools/backfill_summary_experience.py`，它会从真实 trial 重新调用 Summary Agent。查询时没有索引或没有匹配结果就返回 `found: false`，不扫描旧目录或尝试兼容其他格式。
...
python tools/backfill_summary_experience.py --quiet

# quiet 只关闭 Agent token 流；脚本仍会即时打印 starting/running/created 等进度

# 只处理指定日期；已有正式总结默认跳过，显式覆盖时加 --overwrite
python tools/backfill_summary_experience.py --date 0903_1333 --quiet
