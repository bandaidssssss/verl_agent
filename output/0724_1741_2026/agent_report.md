...
**生成时间**: 2026-07-30 11:30:43
**数据来源**: `/Users/noone/Desktop/share/ssh_agent/output/0724_1741_2026`
**总 Trial 数**: 6
...
|---|---|---|---:|---:|---:|:---:|:---:|
| 1 | hardware_tuning | success | 590 | - | 77.5% | - | 0 + 5 + 4 = 9 |
| 2 | hardware_tuning | success | 610 | - | 82.4% | - | 0 + 6 + 3 = 9 |
| 3 | hardware_tuning | success | 605 | - | 84.3% | - | 0 + 6 + 9 = 15 |
| 4 | hardware_tuning | success | 616 | - | 86.7% | - | - |
| 5 | stability_tuning | success | 610 | - | 87.2% | - | 0 + 5 + 2 = 7 |
| 6 | stability_tuning | success | 576 | - | 91.8% | 1 | - |
...
|---|---:|---:|---:|---:|---:|---:|
| rollout | 546.9 | 64.9 | 70.5 | 58.5 | 70.0 | 74.0 |
| actor_log_prob | 141.1 | 37.6 | 77.5 | 53.7 | 74.0 | 80.0 |
| ref_log_prob | 96.8 | 59.9 | 64.5 | 64.6 | 76.0 | 80.0 |
| training | 322.8 | 58.2 | 61.5 | 57.1 | 61.0 | 64.0 |

- **相对最高显存阶段**: actor_log_prob （仅表示四阶段中峰值最高，不等于存在显存压力）
- **总体实测峰值**: 77.5%
...
| 轮次 | 工具 | 参数 / 查询内容 | 状态 |
|---|---|---|---|
| 1 | `parameter_understanding` | actor_rollout_ref.rollout.gpu_memory_utilization, actor_rollout_ref.rollout.max_num_batched_tokens, actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.actor.megatron.tensor_model_parallel_size, actor_rollout_ref.rollout.tensor_model_parallel_size, actor_rollout_ref.actor.megatron.param_offload …共 8 个 | success |
| 1 | `tuning_strategies` | end_to_end_throughput, low_gpu_utilization | success |
| 2 | `memory_estimator` | ref_trial=1, 预测参数: actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens, actor_rollout_ref.rollout.gpu_memory_utilization | success |
| 3 | `memory_estimator` | ref_trial=1, 预测参数: actor_rollout_ref.rollout.max_num_batched_tokens, actor_rollout_ref.rollout.max_num_seqs | success |
| 4 | `memory_estimator` | ref_trial=1, 预测参数: actor_rollout_ref.rollout.gpu_memory_utilization, actor_rollout_ref.rollout.max_num_batched_tokens, actor_rollout_ref.rollout.max_num_seqs | success |
...
| 参数 | 旧值 | 新值 | 原因 |
|---|---|---|---|
| `actor_rollout_ref.rollout.gpu_memory_utilization` | `0.6` | `0.7` | Boost GPU memory share for rollout to accommodate larger KV cache and higher concurrency; rollout memory headroom is ample. |
| `actor_rollout_ref.rollout.max_num_seqs` | `256` | `384` | Increase concurrent sequences in rollout to improve scheduling and GPU utilization without exceeding safe memory boundaries. |
| `actor_rollout_ref.rollout.max_num_batched_tokens` | `65536` | `81920` | Allow larger token batches to enhance prefill/decode utilization and reduce scheduling waves in the bottleneck rollout phase. |
...
| 轮次 | 工具 | 参数 / 查询内容 | 状态 |
|---|---|---|---|
| 1 | `parameter_understanding` | actor_rollout_ref.rollout.gpu_memory_utilization, actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens | success |
| 1 | `memory_estimator` | ref_trial=1, 预测参数: actor_rollout_ref.rollout.gpu_memory_utilization, actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens | success |
...
| `actor_rollout_ref.rollout.max_num_batched_tokens` | `65536` | `81920` |
| `actor_rollout_ref.rollout.max_num_seqs` | `256` | `384` |
...
| 阶段 | 耗时均值 (s) | 显存均值 (%) | 显存峰值 (%) | GPU 利用率均值 (%) | GPU 利用率 P95 (%) | GPU 利用率峰值 (%) |
|---|---:|---:|---:|---:|---:|---:|
| rollout | 508.7 | 74.9 | 82.4 | 60.9 | 72.0 | 74.0 |
| actor_log_prob | 141.2 | 37.5 | 77.5 | 53.6 | 74.0 | 80.0 |
| ref_log_prob | 96.8 | 60.0 | 64.7 | 64.5 | 76.0 | 79.0 |
| training | 322.3 | 58.4 | 61.7 | 57.1 | 61.0 | 63.0 |
...
| 轮次 | 工具 | 参数 / 查询内容 | 状态 |
|---|---|---|---|
| 1 | `parameter_understanding` | actor_rollout_ref.rollout.gpu_memory_utilization, actor_rollout_ref.rollout.max_num_batched_tokens, actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.enable_chunked_prefill, actor_rollout_ref.rollout.enable_prefix_caching, actor_rollout_ref.rollout.enforce_eager …共 8 个 | success |
...
| 2 | `memory_estimator` | ref_trial=2, 预测参数: actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens, actor_rollout_ref.rollout.gpu_memory_utilization | success |
| 3 | `memory_estimator` | ref_trial=2, 预测参数: actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens | success |
| 4 | `memory_estimator` | ref_trial=2, 预测参数: actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens | success |
...
| 参数 | 旧值 | 新值 | 原因 |
|---|---|---|---|
| `actor_rollout_ref.rollout.max_num_seqs` | `384` | `448` | Raise concurrent sequences to improve rollout GPU utilization and reduce scheduling gaps; the prior increase from 256→384 already improved throughput without memory issues. |
| `actor_rollout_ref.rollout.max_num_batched_tokens` | `81920` | `92160` | Scale batched tokens proportionally to support higher concurrency and avoid prefill becoming a new bottleneck; keeps the ratio of batched tokens to max_seqs stable. |
...
| 轮次 | 工具 | 参数 / 查询内容 | 状态 |
|---|---|---|---|
| 1 | `parameter_understanding` | actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens | success |
| 1 | `memory_estimator` | ref_trial=2, 预测参数: actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.max_num_batched_tokens | success |
...
| 参数 | 旧值 | 新值 |
|---|---|---|
| `actor_rollout_ref.rollout.max_num_batched_tokens` | `81920` | `92160` |
| `actor_rollout_ref.rollout.max_num_seqs` | `384` | `448` |
...
| 阶段 | 耗时均值 (s) | 显存均值 (%) | 显存峰值 (%) | GPU 利用率均值 (%) | GPU 利用率 P95 (%) | GPU 利用率峰值 (%) |
|---|---:|---:|---:|---:|---:|---:|
| rollout | 519.1 | 74.5 | 84.3 | 61.9 | 72.0 | 73.0 |
| actor_log_prob | 141.1 | 38.3 | 77.6 | 53.7 | 74.0 | 80.0 |
| ref_log_prob | 96.8 | 60.9 | 64.8 | 65.8 | 76.0 | 79.0 |
| training | 322.1 | 58.5 | 61.8 | 57.2 | 61.0 | 63.0 |
...
| 轮次 | 工具 | 参数 / 查询内容 | 状态 |
|---|---|---|---|
| 1 | `parameter_understanding` | actor_rollout_ref.rollout.gpu_memory_utilization, actor_rollout_ref.rollout.max_num_batched_tokens, actor_rollout_ref.rollout.max_num_seqs, actor_rollout_ref.rollout.tensor_model_parallel_size, actor_rollout_ref.actor.use_dynamic_bsz, actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu …共 8 个 | success |
| 1 | `tuning_strategies` | end_to_end_throughput, low_gpu_utilization | success |
...
| 参数 | 旧值 | 新值 | 原因 |
|---|---|---|---|
| `actor_rollout_ref.rollout.gpu_memory_utilization` | `0.7` | `0.75` | 为 rollout 引擎分配更多 GPU 显存，以容纳更大 KV 缓存、提升并发调度效率，从而缩短瓶颈阶段耗时；预测 rollout 峰值显存从 82.4% 升至 85.8%，仍有 6.2% 余量。 |
...
| 2 | `parameter_understanding` | actor_rollout_ref.actor.megatron.tensor_model_parallel_size | success |
...
| `actor_rollout_ref.rollout.max_num_batched_tokens` | `92160` | `81920` |
| `actor_rollout_ref.rollout.max_num_seqs` | `448` | `384` |
...
| 阶段 | 耗时均值 (s) | 显存均值 (%) | 显存峰值 (%) | GPU 利用率均值 (%) | GPU 利用率 P95 (%) | GPU 利用率峰值 (%) |
|---|---:|---:|---:|---:|---:|---:|
| rollout | 498.5 | 80.1 | 86.7 | 61.4 | 73.0 | 75.0 |
| actor_log_prob | 141.1 | 38.1 | 78.0 | 53.6 | 74.0 | 80.0 |
| ref_log_prob | 96.5 | 61.1 | 64.7 | 65.9 | 76.0 | 80.0 |
| training | 322.9 | 58.4 | 61.7 | 57.1 | 61.0 | 63.0 |
...
| 阶段 | 耗时均值 (s) | 显存均值 (%) | 显存峰值 (%) | GPU 利用率均值 (%) | GPU 利用率 P95 (%) | GPU 利用率峰值 (%) |
|---|---:|---:|---:|---:|---:|---:|
| rollout | 392.9 | 80.8 | 87.2 | 60.4 | 71.0 | 74.0 |
| actor_log_prob | 126.7 | 37.8 | 80.9 | 52.1 | 73.0 | 80.0 |
| ref_log_prob | 80.8 | 15.6 | 16.0 | 0.0 | 0.0 | 0.0 |
| training | 288.4 | 58.1 | 61.7 | 54.6 | 61.0 | 63.0 |
...
| 3 | `memory_estimator` | ref_trial=5, 预测参数: actor_rollout_ref.actor.optim.lr, actor_rollout_ref.actor.entropy_coeff | success |
...
| 参数 | 旧值 | 新值 | 原因 |
|---|---|---|---|
| `actor_rollout_ref.actor.optim.lr_warmup_steps` | `0` | `10` | 从 0 步预热增加至 10 步，平滑训练早期学习率上升，降低 reward 初始波动的风险。 |
| `actor_rollout_ref.actor.entropy_coeff` | `0.0` | `0.01` | 当前 entropy 均值为 0.20，适度施加 0.01 的熵正则化系数可鼓励策略探索，有助于缓解 reward 负向漂移。 |
...
| 轮次 | 工具 | 参数 / 查询内容 | 状态 |
|---|---|---|---|
| 1 | `parameter_understanding` | actor_rollout_ref.actor.optim.lr_warmup_steps, actor_rollout_ref.actor.entropy_coeff | success |
| 2 | `memory_estimator` | ref_trial=5, 预测参数: actor_rollout_ref.actor.optim.lr_warmup_steps, actor_rollout_ref.actor.entropy_coeff | success |
...
| 参数 | 旧值 | 新值 |
|---|---|---|
| `actor_rollout_ref.actor.entropy_coeff` | `0.0` | `0.01` |
| `actor_rollout_ref.actor.optim.lr_warmup_steps` | `0` | `10` |
...
| 阶段 | 耗时均值 (s) | 显存均值 (%) | 显存峰值 (%) | GPU 利用率均值 (%) | GPU 利用率 P95 (%) | GPU 利用率峰值 (%) |
|---|---:|---:|---:|---:|---:|---:|
| rollout | 429.0 | 81.9 | 91.8 | 61.4 | 73.0 | 75.0 |
| actor_log_prob | 128.5 | 38.1 | 81.6 | 51.9 | 74.0 | 80.0 |
| ref_log_prob | 81.1 | 50.7 | 64.7 | 51.0 | 76.0 | 80.0 |
| training | 298.2 | 59.4 | 63.0 | 55.0 | 61.0 | 65.0 |
