...
14. When a binding `max_num_seqs` analysis returns `queue_demand_p95_target`, reject a smaller target justified only by a generic increment or multiplier. A smaller target is acceptable only when concrete estimator or resource evidence establishes an upper bound below the queue-demand target.
