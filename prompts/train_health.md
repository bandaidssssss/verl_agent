...
10. Before returning `observe` or `stop`, call `read_current_trial_metrics` when it is available. First inspect a coarse view of the trajectory, including the automatically returned MATH validation `evaluation.steps`, then use `window_size: 1` around the trigger when the coarse view is ambiguous. A tool failure is missing evidence, not evidence that training is healthy.
