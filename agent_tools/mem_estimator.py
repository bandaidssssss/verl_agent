...
def api(args, num_microbatches=None, verbose=False):
    weight_and_optimizer_memory = (
    ...
    if args.sequence_parallel and args.recompute_granularity == 'selective':
        if verbose:
            print("compute_activation_memory with SP")
        activation_memory = (
        ...
        if verbose:
            print("compute_activation_memory_without_sp")
        activation_memory = (
...
DEFAULT_ARGS = {
    ...
    "kv_channels": None,
    # MLA: need only if multi_latent_attention==true
    "q_lora_rank": None,
    "qk_head_dim": 128,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Estimate training memory usage")
    parser.add_argument("--describe", action="store_true", help="Print skill parameter schema")
    parser.add_argument("--config", type=str, default="{}", help="JSON config overrides")
    parser.add_argument("--verbose", action="store_true")
    cli_args = parser.parse_args()

    if cli_args.describe:
        print(json.dumps({"param_keys": list(DEFAULT_ARGS.keys())}, ensure_ascii=False))
        raise SystemExit(0)

    overrides = json.loads(cli_args.config)
    args = validate_args(SimpleNamespace(**{**DEFAULT_ARGS, **overrides}))
    weight, activation, total = api(args, verbose=cli_args.verbose)

    if cli_args.verbose:
        print(
            f"Theoretical memory footprints: weight and optimizer={weight:.2f} MB, "
            f"activation={activation:.2f} MB, total={total:.2f} MB\n"
        )
    else:
        print(json.dumps({
            "weight_and_optimizer_memory_mb": round(weight, 2),
            "activation_memory_mb": round(activation, 2),
